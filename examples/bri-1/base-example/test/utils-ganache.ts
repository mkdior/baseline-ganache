	...
	public async compileContracts(contracts: ContractSet): Promise<any> {
		...
		for (var contract of contracts) {
			...
			if (contract.params) {
				...
				unsignedTx.data = unsignedTx.data + (this.abiCoder.encode(typeNames, typeValues)).slice(2).toString();
			}

			let gasEstimate = await this.signer.estimateGas(unsignedTx);
			unsignedTx.gasLimit = Math.ceil(Number(gasEstimate) * 1.1);

			let txHash = await this.signer.sendTransaction(unsignedTx)
			.then((txResult: any) => txResult.hash)
			.catch((txError: any) => Promise.reject(JSON.stringify(txError, undefined, 2)));
