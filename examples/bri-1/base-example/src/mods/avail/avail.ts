				...
				fs.readdir(`moduleAvail/${folder}`, (error: any, files: any) => {
					...
					if (files) {
						for (const file of files) {
							...
							switch (folder) {
								case "Ports": {
									contentQueue.push(fileReader(`moduleAvail/${folder}/${file}`, SupplierType.PORT));
									break;
								}
								case "Spares": {
									contentQueue.push(fileReader(`moduleAvail/${folder}/${file}`, SupplierType.SPARES));
									break;
								}
								case "Technicians": {
									contentQueue.push(fileReader(`moduleAvail/${folder}/${file}`, SupplierType.TECHNICIAN));
									break;
								}
								case "Vessels": {
									contentQueue.push(fileReader(`moduleAvail/${folder}/${file}`, SupplierType.VESSEL));
									break;
...
let suitabilityFinder = (suppInfo: number[], timeWindow: number[], taskLength: number): number[] => {
...
const supplierResolver = (suppliers: SupplierType[]): string[] => {
	...
	return suppliers.reduce((pVal: any, cVal: any) => [...pVal, SupplierType[cVal].toLocaleLowerCase() + "s"], []);
};

export const requestAvailability = async (suppliers: SupplierType[], timeWindow: number[], taskLength: number): Promise<FileStructure[]> => {
	...
	await Promise.all(folderInfo).then(async (res: (Contents)[]) => {
		new Promise((resolve, reject) => {
			res.map((entry: Contents) => {
				...
				//FileContentStructure
				let entryAvailability = suitabilityFinder(entryData[0].split(',').map((day: string) => parseInt(day) || 0), timeWindow, taskLength);
		...
		}).then((e: any) => { Promise.resolve(suppFormatted) });
	});
